...
. 如果在配置中使用了自加密驱动器、并且您已设置 `kmip.init.maxwait` 变量设置为 `off` (例如、在使用AFF A900或FAS9500控制 器和NVRAM模块的_Boot node2中、 link:boot_node2_with_a900_controller_and_nvs.html#boot_node2_step1["第 1 步"])、您必须取消设置变量：
+
