...
.步骤
. `sA900_boot_node1]]如果此配置正在使用NetApp存储加密(NSE)、则必须将` etenv bootarg.storageencryption.support `命令设置为`true`、并且必须将`kmip.init.maxwait`变量设置为` off、以避免在加载node1配置后出现启动环路：
+
