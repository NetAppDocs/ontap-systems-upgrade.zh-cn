...
部分或全部聚合已从 node1 重新定位到 node2，HA 仍处于启用状态。Node2 接管 node1 的根卷和任何未重新定位的非根聚合。

.关于此任务
已重新定位的聚合的所有权与已接管的非根聚合的所有权相同，因为主所有者未更改。当 node1 进入 `waiting for giveback state`时，node2 返回所有 node1 非根聚合。

.步骤
. 完全的link:relocate_non_root_aggr_node1_node2.html#relocate-step1["第 1 步"]在“将非根聚合从节点 1 迁移到节点 2”部分再次执行。
. 继续执行节点对升级操作步骤。
...
.关于此任务
Node1 接管 node2 的所有聚合以及它自己已重新定位到 node2 的任何聚合。当 node2 进入 `Waiting for Giveback`状态时，node1 返回 node2 的所有聚合。
...
Node3 不会在 node2 崩溃后接管，因为 HA 已被禁用。
...
Node2 已将其部分或全部聚合重新定位到 node3，后者提供来自已重新定位的聚合的数据。已禁用 HA。
...
.步骤
. 启动 node2 。
. 通过完成以下操作来重新安置剩余的骨料link:relocate_non_root_aggr_node2_node3.html#verify-partner-sys-id["第 1 步"]通过link:relocate_non_root_aggr_node2_node3.html#relocate-step5["第 5 步"]在“将非根聚合从节点 2 迁移到节点 3”部分中。
. 继续执行节点对升级操作步骤。
...
如果 node3 在 node2 将聚合重新定位到 node3 时崩溃，node2 中止任何剩余聚合的重新定位。
...
.步骤
. 启动 node3 。
. 完全的link:relocate_non_root_aggr_node2_node3.html#relocate-step5["第 5 步"]再次在“将非根聚合从节点 2 迁移到节点 3”部分中。
. 继续执行节点对升级操作步骤。
...
.步骤
. 启动 node4 。
. 通过再次完成操作，重新定位属于节点 2 的剩余聚合。link:relocate_node2_non_root_aggr_node3_node4.html#man_relocate_3_4_Step1["第 1 步"]通过link:relocate_node2_non_root_aggr_node3_node4.html#step3["第 3 步"]将节点 2 的非根聚合从节点 3 迁移到节点 4。
. 继续执行节点对升级操作步骤。
