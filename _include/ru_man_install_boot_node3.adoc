...
. 【man_install3_step16]]设置 `bootarg.storageencryption.support` 和 `kmip.init.maxwait` 用于避免在加载node1配置后启动环路的变量。
+
...
WARNING: *一旦HA对上的控制器升级完成、您必须取消设置 `kmip.init.maxwait` 变量。请参见 link:ensure_controllers_set_up_correctly.html#unset_maxwait_manual["确认已正确设置新控制器、步骤11"]。否则、如果发生断电、可能会导致数据丢失。*
