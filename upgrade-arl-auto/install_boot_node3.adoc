...
+
如果您没有FC或UTA/UTA2配置、请执行link:reassign-node1-disks-to-node3.html#reassign-node1-node3-step1["将node1磁盘重新分配给node3、步骤1"]、以便node3可以识别node1的磁盘。

. [[auto9597_istall3_stEP29]]对于MetroCluster配置、V系列系统以及使用FlexArray虚拟化软件连接到存储阵列的系统，请转至link:reassign-node1-disks-to-node3.html#reassign-node1-node3-step1["将node1磁盘重新分配给node3、步骤1"]。
