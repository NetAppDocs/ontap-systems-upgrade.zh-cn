...
如果要使用 NetApp Aggregate Encryption (NAE) 或 NetApp Volume Encryption (NVE) 来加密要升级的系统上的卷，则必须将加密配置同步到新节点。如果不还原密钥管理器，则当使用 ARL 将 node1 聚合从 node2 重新定位到 node3 时，将使加密卷脱机。
