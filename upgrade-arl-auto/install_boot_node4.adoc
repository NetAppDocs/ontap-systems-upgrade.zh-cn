...
. [[auto_install4_step27]]如果您安装了 NetApp 存储加密 (NSE) 驱动器，请执行以下步骤：
+
...
+
如果您没有FC或UTA/UTA2配置、请执行link:reassign-node2-disks-to-node4.html#reassign-node2-node4-step1["将node2磁盘重新分配给node4、步骤1"]、以便node4可以识别node2的磁盘。

. [[Step29]]对于MetroCluster配置、V系列系统以及使用FlexArray虚拟化软件连接到存储阵列的系统，请转至link:reassign-node2-disks-to-node4.html#reassign-node2-node4-step1["将node2磁盘重新分配给node4、步骤1"]。
