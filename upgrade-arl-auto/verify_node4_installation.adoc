...
CAUTION: 此时，MetroCluster 配置未处于正常状态，您可能需要解决错误。请参阅 link:verify_health_of_metrocluster_config.html["验证 MetroCluster 配置的运行状况。"]。
...
如果要使用 NetApp Aggregate Encryption (NAE) 或 NetApp Volume Encryption (NVE) 来加密要升级的系统上的卷，则必须将加密配置同步到新节点。如果不还原密钥管理器，则当使用 ARL 将 node2 聚合从 node3 重新定位到 node4 时，将使加密卷脱机。
