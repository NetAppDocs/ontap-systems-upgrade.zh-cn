...
.关于此任务
您可能需要填写<<configure-fc-ports-on-node3,在 node3 上配置 FC 端口>>或<<check-config-uta-uta2-node3,检查并配置 node3 上的 UTA/UTA2 端口>>，或者同时填写这两个部分。
...
.关于此任务
如果您的系统没有 FC 配置，您可以跳过此部分。如果您的系统具有板载 UTA/UTA2 端口或 UTA/UTA2 卡，请在 <<check-config-uta-uta2-node3,检查并配置 node3 上的 UTA/UTA2 端口>> 中进行配置。
...
. 执行以下操作之一：
+
** 如果 node3 具有 UTA/UTA2 卡或 UTA/UTA2 板载端口，请转到 <<check-config-uta-uta2-node3,检查并配置 node3 上的 UTA/UTA2 端口>>。
** 如果 node3 没有 UTA/UTA2 卡或 UTA/UTA2 板载端口，请跳过 <<check-config-uta-uta2-node3,检查并配置 node3 上的 UTA/UTA2 端口>> 并转到 link:reassign-node1-disks-to-node3.html["将node1磁盘重新分配给node3"]。
...
== 步骤 2：检查并配置 node3 上的 UTA/UTA2 端口
