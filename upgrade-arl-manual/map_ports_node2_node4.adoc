...
.. 验证新集群端口是否位于集群广播域中：
+
`network port show -node -port port -fields broadcast-domain` 以下示例显示端口 "e0a" 位于 node4 上的集群域中：
...
+
`MetroCluster SVM resync -vserver vserver_name`
...
. 【 man_map_2_step5]] 调整托管集群 LIF 和节点管理 / 集群管理 LIF 的广播域。确保每个广播域包含正确的端口。如果某个端口托管 LIF 或位于 LIF 的主目录中，则无法在广播域之间移动该端口，因此您可能需要迁移和修改 LIF ，如以下步骤所示：
+
.. 显示 LIF 的主端口：
+
...
. 调整所有 LIF 故障转移组： `network interface modify -failover-group failover_group -failover-policy failover_policy`
+
...
+
有关详细信息，请参见 link:other_references.html["参考资料"] 要链接到 _LIF ONTAP 9 网络管理指南 _ 或 _LIF ONTAP 9 命令：手册页参考 _ ，请转至 _Configuring failover settings on a LIF_ 。
