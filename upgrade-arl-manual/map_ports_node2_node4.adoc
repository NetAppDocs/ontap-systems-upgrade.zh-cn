...
[role="lead"]
node2 上的物理端口必须正确映射到 node4 上的物理端口。这允许 node4 在升级后与集群中的其他节点以及网络进行通信。
...
因此，如果 node2 上的物理端口未直接映射到 node4 上的物理端口，则需要进行软件配置更改才能在启动后恢复集群、管理和网络连接。此外，如果 node2 上的集群端口没有直接映射到 node4 上的集群端口，则 node4 在重新启动时可能不会自动重新加入仲裁，直到进行软件配置更改以将集群 LIF 托管在正确的物理端口上。
...
+
有关详细信息，请参见 link:other_references.html["参考资料"] to link to _Network Management_ 或 _ONTAP 9 Command reference_，并转到 _Configuring failover settings on a LIF_。
