...
+
有关 `storage aggregate relocation start`、 `override-vetoes`和 `override-destination-checks`选项的详细信息，请参见link:other_references.html["参考资料"]以链接到 _ONTAP 9 Command reference_。
...
* 在阶段 3 中，聚合从 node2 重新定位到 node3。正在重新定位的一些聚合的主节点为 node1。例如，这样的聚合可以称为 aggr_node_1。如果在阶段 3 期间 aggr_node_1 的重定位失败，并且无法强制重定位，则聚合将留在 node2 上。
* 阶段 4 之后，当 node2 替换为 node4 时。当 node2 被替换时，aggr_node_1 以 node4 而不是 node3 作为其主节点上线。
