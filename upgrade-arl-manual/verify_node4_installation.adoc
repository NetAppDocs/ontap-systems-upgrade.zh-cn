...
| 与 node3 位于同一机箱中 | 转至 ,第 5 步。您无需在节点之间手动连接 HA 互连；在同机箱配置中， HA 互连会通过背板自动连接。 
|===
. 【第 5 步】执行以下操作之一：
+
[cols="35,65"]
...
| 在 SAN 环境中 | 完成 ,第 6 步 然后转到部分 link:move_nas_lifs_node2_from_node3_node4_verify_san_lifs_node4.html["将 node2 拥有的 NAS 数据 LIF 从 node3 移动到 node4 并验证 node4 上的 SAN LIF"]。 
