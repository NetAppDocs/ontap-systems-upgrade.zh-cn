...
您必须使原始节点的端口和 LIF 配置与您计划新节点的配置兼容。这是因为新节点在启动时会重放相同的配置，这意味着当您启动 node3 时，ONTAP 尝试在 node1 上使用的相同端口上托管 LIF。

因此，如果 node1 上的物理端口未直接映射到 node3 上的物理端口，则需要进行软件配置更改才能在启动后恢复集群、管理和网络连接。此外，如果 node1 上的集群端口没有直接映射到 node3 上的集群端口，则 node3 在重新启动时可能不会自动重新加入仲裁，直到软件配置更改为在正确的物理端口上托管集群 LIF。
...
|===
+
有关获取此信息的步骤，请参见 link:record_node1_information.html["记录 node1 信息"]。
...
----
+
转到 link:other_references.html["参考资料"] 以链接到 _Network Management_ 或 _ONTAP 9 Command reference_ 获取更多信息。
