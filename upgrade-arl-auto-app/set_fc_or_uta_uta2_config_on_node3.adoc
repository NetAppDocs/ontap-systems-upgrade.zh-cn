...
** 如果 node3 没有 UTA/UTA2 卡或 UTA/UTA2 板载端口，请跳过部分  and configure UTA/UTA2 ports on node3 然后转到部分 link:verify_node3_installation.html["验证 node3 安装"]。
...
** 如果 node3 具有 UTA/UTA2 卡或 UTA/UTA2 板载端口，请转至部分  and configure UTA/UTA2 ports on node3。
** 如果 node3 没有 UTA/UTA2 卡或 UTA/UTA2 板载端口，请跳过部分  and configure UTA/UTA2 ports on node3 并返回到 _Install and boot node3_ 部分，然后在继续 link:install_boot_node3.html#step23["第 23 步"]。
...
. 如果系统进入重新启动循环并显示消息 `no disks found` ，则表示系统已将 FC 或 UTA/UTA2 端口重置回目标模式，因此无法看到任何磁盘。要解决此问题，请继续 ,第 17 步 to ,第 22 步或转到第节 link:verify_node3_installation.html["验证 node3 安装"]。
