...
.步骤
. [[reassign-node2-node4-app-step1]]验证node2已在启动菜单停止，并将node2的磁盘重新分配给node4：
+
...
. 如果系统进入重启循环并显示以下消息 `no disks found`，表示系统已将 FC 或 UTA/UTA2 端口重置回目标模式，因此无法看到任何磁盘。选择以下任务之一来解决此问题：
+
** 履行<<reassign-node2-node4-app-step3,第 3 步>>到<<reassign-node2-node4-app-step8,第 8 步>>在节点4上
** 前往章节link:verify_node4_installation.html["验证 node4 安装"]
...
. [[reassign-node2-node4-app-step9]] 如果要从具有外部磁盘的系统升级到支持内部和外部磁盘的系统（例如AFF A800系统），请将 node2 聚合设置为根聚合，以确保 node4 从 node2 的根聚合启动。要设置根聚合，请转到 node4 上的启动菜单并选择选项 `5`进入维护模式。
+
...
.下一步是什么？
将 node2 磁盘重新分配给 node4 后，link:verify_node4_installation.html["验证 node4 安装"]。
