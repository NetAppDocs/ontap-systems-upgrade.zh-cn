...
您可能需要完成  FC ports on node4 部分  and configure UTA/UTA2 ports on node4或这两个部分。
...
| 是 V 系列系统或具有 FlexArray 虚拟化软件并连接到存储阵列 | 根据需要修改node4上的FC端口：`ucadmin modify -m fc -t initiator -f _adapter_port_name_``-t`是FC4类型、目标或启动程序。* 注 * ：必须将 FC 端口编程为启动程序。 
|===
. 退出维护模式：
+
`halt`
...
. 如果系统进入重新启动循环并显示消息 `no disks found` ，则表示系统已将 FC 或 UTA/UTA2 端口重置回目标模式，因此无法看到任何磁盘。要解决此问题，请继续 ,第 17 步 to ,第 22 步 或者转到第节 link:verify_node4_installation.html["验证 node4 安装"]。
