...
.关于此任务
您可能需要填写<<configure-fc-ports-on-node4,在节点 4 上配置 FC 端口>>或<<check-config-uta-uta2-node4,检查并配置 node4 上的 UTA/UTA2 端口>>，或者同时填写这两个部分。
...
.关于此任务
如果您的系统没有 FC 配置，您可以跳过此部分。如果您的系统具有板载 UTA/UTA2 端口或 UTA/UTA2 适配器，请在 <<check-config-uta-uta2-node4,检查并配置 node4 上的 UTA/UTA2 端口>> 中进行配置。
...
. 【第 8 步】执行以下操作之一：
+
** 如果 node4 具有 UTA/UTA2 卡或 UTA/UTA2 板载端口，请转到 <<check-config-uta-uta2-node4,检查并配置 node4 上的 UTA/UTA2 端口>>。
...
== 步骤 2：检查并配置 node4 上的 UTA/UTA2 端口
