...
. [[unset_maxwait_system_commands]]如果在配置中使用自加密驱动器并且您已设置 `kmip.init.maxwait`变量 `off`（例如，在link:install_boot_node4.html#auto_install4_step22["安装并启动 node4，步骤 22"]），您必须取消设置该变量：
+
