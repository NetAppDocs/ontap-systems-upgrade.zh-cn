...
* 如果要升级具有存储磁盘的系统，则需要完成整个部分，然后转到 link:set_fc_or_uta_uta2_config_on_node3.html#configure-fc-ports-on-node3["在 node3 上配置 FC 端口"] 和 link:set_fc_or_uta_uta2_config_on_node3.html#check-config-uta-uta2-node3["检查并配置 node3 上的 UTA/UTA2 端口"] 部分，在集群提示符下输入命令。
...
.下一步是什么？
* 如果您的系统具有 FC 或 UTA/UTA2 配置，link:set_fc_or_uta_uta2_config_on_node3.html["设置并配置节点 3 上的 FC 或 UTA/UTA2 端口"] 。
* 如果您没有 FC 或 UTA/UTA2 配置，link:reassign-node1-disks-to-node3.html#reassign-node1-node3-app-step1["将 node1 磁盘重新分配给 node3，步骤 1"]这样node3就可以识别node1的磁盘。
* 如果您有MetroCluster配置，link:set_fc_or_uta_uta2_config_on_node3.html["设置并配置节点 3 上的 FC 或 UTA/UTA2 端口"]检测连接到节点的磁盘。
