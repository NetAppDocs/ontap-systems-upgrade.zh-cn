...
.步骤
. [[reassign-node1-node3-app-step1]]验证 node1 是否已在启动菜单处停止。将node1的磁盘重新分配给node3：
+
...
+
** 履行<<reassign-node1-node3-app-step3,第 3 步>>到<<reassign-node1-node3-app-step8,第 8 步>>在节点3上
** 前往章节link:verify_node3_installation.html["验证 node3 安装"]
...
.下一步是什么？
将 node1 磁盘重新分配给 node3 后，link:verify_node3_installation.html["验证 node3 安装"]。
