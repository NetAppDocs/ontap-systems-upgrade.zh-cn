...
[role="lead"]
您必须验证 node2 的物理端口是否正确映射到 node4 上的物理端口。这使 node4 能够在升级后与集群中的其他节点以及网络进行通信。

.关于此任务
请参见 link:other_references.html["参考资料"] 链接到 _Node_ Hardware Universe 以捕获有关新节点上端口的信息。您将在本节稍后部分使用此信息。

物理端口布局可能会有所不同，具体视节点的型号而定。当新节点启动时，ONTAP 尝试确定哪些端口应托管集群 LIF，以便自动进入仲裁。

如果 node2 上的物理端口没有直接映射到 node4 上的物理端口，则必须使用后续部分 <<restore_network_configuration_on_node4,还原 node4 上的网络配置>> 来修复网络连接。

安装并启动 node4 后，您必须验证是否已正确安装它。您必须等待 node4 加入仲裁，然后恢复重新定位操作。

在此过程中的此时，当 node4 加入仲裁时，操作暂停。



== 步骤 1：验证 node4 是否处于法定状态

验证 node4 是否处于仲裁状态。然后，恢复控制器更换操作
...
| 9.12.1或更高版本 | 跳过此步骤并转到 <<verify_node4_step5,第 5 步>>。 
|===
+
...
+
在执行可达性修复时，ONTAP 尝试将端口放置在正确的广播域中。但是，如果 ONTAP 无法确定端口的可达性，并且它不属于任何现有广播域，则 ONTAP 会为这些端口创建新的广播域。
...
.下一步是什么？
在验证 node4 安装后，link:restore_key-manager_config_node4.html["在 node4 上恢复 key-manager 配置"]。
