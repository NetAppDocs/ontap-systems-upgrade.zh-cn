...
NOTE: 如果使用板载密钥管理器(Onboard Key Manager、OKM)的NetApp卷加密(NetApp Volume Encryption、NVE)或NetApp聚合加密(NetApp Aggregate Encryption、NAE)、请准备好密钥管理器密码短语、以便稍后在操作步骤 中完成密钥管理器重新同步。
