...
+
`ssystem controller replace start -nodes <node_name> -NSO false`
...
NOTE: 如果您的系统每个节点具有两个以上的集群端口，例如 FAS8080 或 AFF8080 ，则必须先将集群 LIF 整合为每个节点两个集群端口，然后再开始控制器升级。如果在每个节点上执行控制器升级时使用两个以上的集群端口，则在升级后，新控制器上可能会缺少集群 LIF 。
...
** `ssystem node show -instance -node node_name`
...
** `ssystem license show -owner node_name`
** `s存储加密磁盘 show`
