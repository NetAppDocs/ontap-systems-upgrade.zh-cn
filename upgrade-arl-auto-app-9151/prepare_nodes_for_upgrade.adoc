...
summary: '使用命令对节点进行准备、并在升级运行ONTAP 9.151或更高版本的控制器时运行一系列预检 `system controller replace` 。' 
---
= 准备要升级的节点
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
控制器更换过程从一系列预先检查开始。您还可以收集有关原始节点的信息，以便在以后的过程中使用。如果任何预检查失败，则必须更正聚合所有权，然后才能继续升级。



== 步骤 1：准备节点

启动控制器更换过程并运行预检查。如果需要，请确定正在使用的自加密驱动器的类型。
...
NOTE: 您只能在高级权限级别执行system controller Replace start命令： `set -privilege advanced`

+
此时将显示类似于以下示例的输出。此输出将显示群集上运行的 ONTAP 版本：
...
[NOTE]
====
如果您的系统(例如700)具有以下配置、则在开始升级之前、您可以将集群AFF迁移并将其重新归位到每个节点的两个集群端口：

** 每个节点两个以上的集群端口
** 以分支模式在插槽4中创建集群互连卡、用于创建端口e4a、e4b、e4c和e4D以及端口e4e、 e4f、e4g和e4h


如果执行控制器升级时每个节点的集群端口数超过两个、则可能会导致升级后新控制器上的集群丢失。
...
.下一步是什么？
准备好节点后，link:complete-preparation-for-upgrade.html["完成升级准备"]。
