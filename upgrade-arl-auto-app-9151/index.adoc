...
[role="lead"]
有几种用于升级控制器硬件的聚合重定位（ARL）方法。此 ARL 过程描述了如何在 ONTAP 9.15.1 或更高版本中引入的 AFF 和 FAS 存储系统中使用新控制器升级 HA 对控制器，同时保留现有磁盘和存储在磁盘上的数据。
...
.自动控制器升级过程
在控制器升级期间，现有控制器将替换为运行更新或更强大平台的另一个控制器。此内容提供了部分自动化程序的步骤，该程序利用自动网络端口可达性检查来进一步简化控制器升级体验。

.下一步是什么？
了解此 ARL 升级过程后，link:decide_to_use_the_aggregate_relocation_guide.html["决定是否使用此程序"]。
