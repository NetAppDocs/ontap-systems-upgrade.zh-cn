...
summary: '在升级运行ONTAP 9.15.1或更高版本的控制器时、使用命令将node2的非根聚合重新定位到node3 `system controller replace` 。' 
---
...
.下一步是什么？
将非根聚合和 NAS 数据 LIF 从 node2 重新定位到 node3 后，link:relocate-failed-or-vetoed-aggr-node3.html["将失败或否决的聚合重新定位到 node3"]。
