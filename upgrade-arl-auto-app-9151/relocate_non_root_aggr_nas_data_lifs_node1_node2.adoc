...
summary: '使用命令升级运行ONTAP 9.151或更高版本的控制器时、将非根聚合和NAS数据LUN从node1移动到node2、并将node1的资源移动到node3 `system controller replace` 。' 
---
...
.下一步是什么？
将 node1 所拥有的非 root 聚合和 NAS 数据 LIF 重新定位到 node2 后，link:relocate-failed-or-vetoed-aggr-node2.html["将失败或否决的聚合重新定位到 node2"]。
