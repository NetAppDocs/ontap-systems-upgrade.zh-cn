...
. 当 node3 停止在启动菜单时，通过在 node3 上运行以下命令将 node1 磁盘重新分配给 node3：
+
...
. 在自动引导期间输入 `Ctrl-C`以在 `LOADER>`提示符处停止节点。
. 在加载程序提示符处、进入维护模式：
+
`boot_ontap maint`
...
.下一步是什么？
安装并引导 node3 后，link:verify_node3_installation.html["验证 node3 安装"]。
