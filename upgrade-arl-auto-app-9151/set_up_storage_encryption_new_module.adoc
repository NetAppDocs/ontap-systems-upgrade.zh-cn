...
summary: '如果需要、在升级运行ONTAP 9.15.1或更高版本的控制器时、使用命令配置存储加密、安装SSL证书并设置密钥管理服务器 `system controller replace` 。' 
---
= 在新控制器模块上设置存储加密
:hardbreaks:
:allow-uri-read: 
...
.下一步是什么？
在新控制器模块上设置 Storage Encryption 后，link:set_up_netapp_volume_encryption_new_module.html["在新的控制器模块上设置 NetApp 卷或聚合加密"]。
