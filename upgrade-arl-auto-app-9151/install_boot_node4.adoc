...
+
输入您在此操作步骤前面记录的密码短语和备份信息。请参阅 link:complete-preparation-for-upgrade.html#manage_storage_encryption_using_okm["使用板载密钥管理器管理存储加密"]。
...
NOTE: 在以上控制台输出示例中，如果系统使用高级磁盘分区 (ADP) 磁盘，则 ONTAP 会提示您输入配对节点名称。

. [[auto_check_4_step27]]在 LOADER 提示符处，启动节点：
+
`boot_ontap 菜单`
...
.下一步是什么？
安装并引导 node4 后，link:verify_node4_installation.html["验证 node4 安装"]。
