...
[role="lead"]
您必须验证 node1 的物理端口是否正确映射到 node3 上的物理端口。这使 node3 能够在升级后与集群中的其他节点以及网络进行通信。
...
如果 node1 上的物理端口没有直接映射到 node3 上的物理端口，则必须使用后续部分 <<restore_network_configuration_on_node3_9151,还原 node3 上的网络配置>> 来修复网络连接。

安装并启动 node3 后，您必须验证是否已正确安装它。您必须等待 node3 加入仲裁，然后恢复重新定位操作。

在此过程中的此时，当 node3 加入仲裁时，操作暂停。



== 步骤 1：验证 node3 是否处于法定状态

验证 node3 是否处于仲裁状态。然后，恢复控制器更换操作
...
+
在执行可达性修复时，ONTAP 尝试将端口放置在正确的广播域中。但是，如果 ONTAP 无法确定端口的可达性，并且该端口不属于任何现有广播域，则 ONTAP 会为这些端口创建新的广播域。
...
.下一步是什么？
在验证 node3 安装后，link:restore_key-manager_configuration_node3.html["在 node3 上恢复 key-manager 配置"]。
