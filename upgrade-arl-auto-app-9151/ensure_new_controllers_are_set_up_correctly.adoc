...
summary: '在升级运行ONTAP 9.15.1或更高版本的控制器后、使用命令启用HA对、验证node3和node4存储访问并确认聚合所有权 `system controller replace` 。' 
---
= 确认新控制器设置正确
:hardbreaks:
:allow-uri-read: 
...
. [[unset_maxwait_system_commands]]如果在配置中使用自加密驱动器，并且您已将变量设置 `kmip.init.maxwait` 为 `off` (例如中的 link:install_boot_node4.html#auto_install4_step24["安装并启动 node4 步骤 24"])，则必须取消设置变量：
+
...
+
有关 SP 的信息，请参阅 link:other_references.html["参考资料"] 链接到_系统管理参考_，有关 system `service-processor network modify` 命令的详细信息，请参阅 _ONTAP 9 命令参考_。
...
.下一步是什么？
确认新控制器已正确设置后，link:set_up_storage_encryption_new_module.html["在新的控制器模块上设置存储加密"]。
