...
[role="lead"]
有几种用于升级控制器硬件的聚合重定位 (ARL) 方法。此 ARL 过程描述了如何在 ONTAP 9.15.1 或更高版本中引入的 AFF 和 FAS 存储系统中使用新控制器升级 HA 对控制器，同时保留现有磁盘和存储在磁盘上的数据。这是一个复杂的过程，只能由经验丰富的 ONTAP 管理员使用。
...
.下一步是什么？
在决定使用此 ARL 过程后，link:required_tools_and_documentation.html["了解所需的工具和文档"]。
