...
.. 在执行可达性修复时，ONTAP 尝试将端口放置在正确的广播域中。但是，如果 ONTAP 无法确定端口的可达性，并且该端口与任何现有广播域都不对应，则 ONTAP 会为这些端口创建新的广播域。根据需要，如果所有成员端口都将成为接口组的成员端口，则可以删除新创建的广播域。删除广播域：
+
...
. [[map_98_7]]从 ONTAP 9.8 开始，如果端口在网络端口可达性修复过程中在广播域之间移动，ONTAP 会自动修改 LIF 的主端口。如果 LIF 的主端口已移至其他节点或未分配，则该 LIF 将显示为移位 LIF。恢复已移位 LIF 的主端口，这些 LIF 的主端口已不存在或已重新定位到其他节点。
+
