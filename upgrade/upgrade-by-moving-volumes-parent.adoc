...
summary: 如果通过移动卷进行升级，则需要准备原始节点并将新节点加入集群。您可以将卷移动到新节点，配置 LIF，并从集群中删除原始节点。通过移动卷进行升级是一个无中断的过程。 
---
= 通过移动卷工作流程进行升级
:allow-uri-read: 
:icons: font
...
要通过移动卷进行升级，您需要准备好原始节点并将新节点加入集群。您将卷迁移到新节点，配置 LIF，然后从集群中删除原始节点。
...
.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-7.png["七个"]link:upgrade-remove-original-nodes.html["从集群中移除原始节点。"]
[role="quick-margin-para"]
将卷迁移到新节点后，从集群中删除原始节点。删除节点时，节点的配置将被擦除，所有磁盘将被初始化。
