...
---
= 了解如何将 AFF A250 转换为驱动器架并将存储转移到 AFF A400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
执行从 AFF A250 系统到 AFF A400 系统的无中断升级。首先，将 AFF A250 节点转换为 NS224 驱动器架。然后，将它们连接到 AFF A400 替换节点。这会将 AFF A250 板载存储移动到 AFF A400 替换系统。
...
[role="quick-margin-para"]
将 AFF A250 node2 转换为 NS224 驱动器架，然后连接到 AFF A400 node4，然后将驱动器从 node2 重新分配到 node4。
...
[role="quick-margin-para"]
将 AFF A250 node1 转换为 NS224 驱动器架，然后连接到 AFF A400 node3，然后将驱动器从 node1 重新分配到 node3。
