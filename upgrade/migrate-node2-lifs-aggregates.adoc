...
summary: '在将 node2 转换为驱动器架之前，将 node2 上的逻辑接口 (LIF) 和数据聚合迁移到 node1。' 
---
...
[role="lead"]
在将 node2 转换为驱动器架之前，将 AFF A150、AFF A220 或 FAS2820 node2 上的逻辑接口 (LIF) 和数据聚合迁移到 node1。

.开始之前
验证是否满足以下要求：

* 如果可能，原始控制器和替换控制器具有相同的 ONTAP 版本和补丁版本。有关支持的 ONTAP 版本，请参见 https://hwu.netapp.com["NetApp Hardware Universe"^]。
...
.下一步是什么？
link:convert-node2-to-drive-shelf.html["将node2转换为驱动器架并连接到node4"]
