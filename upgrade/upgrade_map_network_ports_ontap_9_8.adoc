...
. [[MAP_98_5]] 修改托管数据 LIF 的物理端口的广播域成员资格。您可以手动执行此操作，如 _Map network ports using ONTAP 9.7 or earlier 中所示， link:upgrade-map-network-ports-ontap-9-7-or-earlier.html#map_97_7["第 7 步"]。建议的方法是使用 ONTAP 9.8 中引入的增强型网络可访问性扫描和修复操作步骤，如以下步骤 5 的子步骤（ a ）到（ g ）所示。
+
.. 列出所有端口的可访问性状态：
+
`网络端口可访问性显示`
