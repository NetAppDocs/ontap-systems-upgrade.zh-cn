...
. 找到您在中记录的端口，广播域和 IP 空间的信息 link:upgrade-prepare-when-moving-storage.html#prepare_move_store_3["移动存储时准备升级，步骤 3"]。
+
...
. [[map_9.7_7]] 调整节点管理广播域，并根据需要迁移节点管理和集群管理 LIF ：
+
.. 显示 LIF 的主端口：
+
...
. 如有必要，请使用中的命令调整集群间广播域并迁移集群间 LIF <<map_9.7_7,第 7 步>>。
. 如果需要，请使用中的命令调整任何其他广播域并迁移数据 LIF <<map_9.7_7,第 7 步>>。
. 调整所有 LIF 故障转移组：
+
