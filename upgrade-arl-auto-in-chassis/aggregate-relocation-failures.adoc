...
+
有关 `storage aggregate relocation start`、 `override-vetoes`和 `override-destination-checks`选项的详细信息，请参见link:other-references.html["参考资料"]以链接到 _ONTAP 9 Command reference_。
...
+
当 node2 被替换时，aggr_node_1 以 node1 而不是 node2 作为其主节点上线。
...
+
要识别未正确重新定位的聚合，请参见您在 link:prepare-nodes-for-upgrade.html["准备要升级的节点"] 部分中获取的 node1 的主所有者的聚合列表，并将其与上述命令的输出进行比较。

. 将步骤 1 的输出与您在 link:prepare-nodes-for-upgrade.html["准备要升级的节点"] 部分中为 node1 捕获的输出进行比较，并记录任何未正确重新定位的聚合。
. 重新定位节点 2 上遗留的聚合：
+
