...
summary: 决定是否使用 ARL 过程来升级相同机箱、磁盘和磁盘上存储的数据中的控制器型号。 
---
= 决定是否使用此聚合迁移程序
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
有几种用于升级控制器硬件的聚合重定位 (ARL) 方法。此内容介绍如何通过将现有系统转换为替换系统、保留现有系统机箱、磁盘和磁盘上存储的数据来升级 HA 对中的存储控制器。仅当您是经验丰富的 ONTAP 管理员时，才应使用此复杂程序。
...
+
要升级 MetroCluster IP 配置，请参见 link:other-references.html["参考资料"] 以链接到 _MetroCluster 升级和扩展_ 内容。
...
下表显示了通过将现有系统转换为替换系统、保留现有系统机箱、磁盘及其上存储的数据来执行控制器升级的支持系统矩阵。


CAUTION: 此过程严格适用于以下升级配置。请勿使用此过程在任何其他系统组合之间执行升级。对于所有其他控制器型号，请参阅 link:other-references.html["参考资料"] 以链接到_使用"system controller replace"命令升级运行 ONTAP 9.8 或更高版本的控制器硬件_和_使用聚合重定位手动升级运行 ONTAP 9.8 或更高版本的控制器硬件_。
...
* 如果您更喜欢升级控制器硬件的其他方法，并且愿意执行卷移动，请参阅 link:other-references.html["参考资料"] 链接到_通过移动卷或存储进行升级_。


.相关信息
请参阅 link:other-references.html["参考资料"] 以链接到 _ONTAP 9 Documentation_。

.下一步是什么？
在决定使用此 ARL 过程后，link:required-tools-and-documentation.html["了解所需的工具和文档"]。
