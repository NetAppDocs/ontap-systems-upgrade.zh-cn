...
. 如果任何卷处于脱机状态，请将其与您在 link:prepare-nodes-for-upgrade.html["准备要升级的节点"] 部分中捕获的脱机卷列表进行比较，并根据需要使用以下命令使任何脱机卷联机，每个卷使用一次：
+
...
+
您可以将输出与在 link:prepare-nodes-for-upgrade.html["准备要升级的节点"] 部分中捕获的输出进行比较。

. [[unset_maxwait]] 如果在配置中使用自加密驱动器，并且已将 `kmip.init.maxwait`变量设置为 `off`（例如，在 _Boot node2 with the replacement system modules_ 中，link:boot-node2-with-a900-controller-and-nvs.html#boot_node2_step1["第 1 步"]），则必须取消设置变量：
+
...
+
有关 SP 的信息，请参阅 link:other-references.html["参考资料"] 链接到_系统管理参考_，有关 system `service-processor network modify` 命令的详细信息，请参阅 _ONTAP 9 命令参考_。

. 如果要在新节点上设置无交换机集群，请参阅 link:other-references.html["参考资料"] 链接到 _NetApp Support Site_ 并按照 _Transitioning to a two-node switchless cluster_ 中的说明进行操作。


.完成后
如果在 node1 和 node2 上启用了 Storage Encryption，请完成此部分link:set-up-storage-encryption-new-module.html["在新控制器模块上设置存储加密"]。否则，请完成此部分link:decommission-old-system.html["停用旧系统"]。

.下一步是什么？
确认新控制器已正确设置后，link:set-up-storage-encryption-new-module.html["在新的控制器模块上设置存储加密"]。
