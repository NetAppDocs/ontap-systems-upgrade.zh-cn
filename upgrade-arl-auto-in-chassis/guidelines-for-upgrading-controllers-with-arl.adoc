...
某些系统配置支持控制器升级。有关支持的系统和最低 ONTAP 版本的列表，请参阅link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems["支持的系统升级组合"]。

如果您作为一个完整的系统收到了新的 AFF A30、AFF A50、AFF A70、AFF A90、AFF A150、AFF A900、AFF C30、AFF C60、AFF C80、FAS2820 或 FAS9500，包括新机箱，请参阅 link:other-references.html["参考资料"] 以链接到_使用"system controller replace"命令升级运行 ONTAP 9.8 或更高版本的控制器硬件_。
...
如果出现任何问题，请参阅过程末尾的 link:aggregate-relocation-failures.html["故障排除"] 部分，以获取更多信息和可能的解决方案。有关可能发生的故障的信息按 link:overview-of-the-arl-upgrade.html["ARL 升级序列"] 中的过程阶段列出。

如果您没有找到所遇到问题的解决方案，请联系技术支持。

.下一步是什么？
在查看升级控制器的准则后，link:overview-of-the-arl-upgrade.html["了解 ARL 升级顺序"]。
