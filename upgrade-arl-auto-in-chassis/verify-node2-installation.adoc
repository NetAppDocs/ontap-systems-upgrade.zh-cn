...
== 步骤 1：验证 node2 是否处于法定状态

验证 node2 是否处于仲裁状态。然后，恢复控制器更换操作。
...
. 当控制器更换处于暂停状态时，请继续 <<restore_network_configuration_on_node2,还原 node2 上的网络配置>>。




== 步骤 2：恢复 node2 上的网络配置
...
.下一步是什么？
在验证 node2 安装后，link:restore-key-manager-config-node2.html["在 node2 上恢复 key-manager 配置"]。
