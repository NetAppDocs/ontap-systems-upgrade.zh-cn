...
---
= 更换 node1 AFF A250 或 AFF C250 控制器模块
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
将 node1 AFF A250 或 AFF C250 控制器模块替换为 AFF A30、AFF A50、AFF C30 或 AFF C60 控制器模块。
...
要卸下 node1 控制器模块，请卸下电缆管理设备并解锁锁闩。然后，从机箱中取出控制器模块。
...
+
如果不按时中断 AUTOBOOT 并且 node1 开始引导，请等待提示，然后输入 Ctrl-C 进入引导菜单。节点停止在引导菜单后，使用选项 `8`重新引导节点，并在重新引导期间中断 AUTOBOOT。
...
.下一步是什么？
安装 AFF A30、AFF A50、AFF C30 或 AFF C60 控制器模块后，link:netboot-node1.html["网络引导 node1"]。
