...
[role="lead"]
具有替换模块的 Node1 现已准备好引导。支持的替换模块列在 link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["支持的系统列表"] 中。
...
+
输入您在此操作步骤前面记录的密码短语和备份信息。请参阅 link:complete-preparation-for-upgrade.html#manage-storage-encryption-using-okm["使用板载密钥管理器管理存储加密"]。
...
.下一步是什么？
使用替换的系统模块引导 node1 后，link:verify-node1-installation.html["验证 node1 安装"]。
