...
除非将更换控制器模块安装在机箱中并接通电源，否则无法检查其启动介质上安装的 ONTAP 版本。更换系统启动介质上的 ONTAP 版本必须与您要升级的旧系统上运行的 ONTAP 版本相同，并且启动介质上的主启动映像和备份启动映像都必须匹配。要验证升级支持的最低 ONTAP 版本，请参见 link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["支持的系统列表"]。
...
* 从 _NetApp Support Site_ 下载系统所需的系统文件和正确版本的 ONTAP。请参阅 link:other-references.html["参考资料"] 以链接到 _NetApp Support Site_。
...
.步骤
. 请参阅 link:other-references.html["参考资料"] 以链接到 _NetApp Support Site_ 下载用于执行系统网络启动的文件。
...
. 清除启动媒体上的所有先前配置。
+
.. 在以下提示符处，运行 `wipeconfig`命令。然后选择 Enter：
+
[listing]
----
Please choose one of the following:
...
.下一步是什么？
在 netbooting node1 之后，link:cable-node1-for-shared-cluster-HA-storage.html["为共享集群 HA 和存储连接 node1"]。
