...
如果要升级到双节点无交换机集群配置，则可以跳过此部分并转到 link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["将非根聚合和 NAS 数据 LIF 移回 node2"]。
...
| `Up` | 转至 link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["将非根聚合和 NAS 数据 LIF 移回 node2"]。 


| `Down` | 前往<<verify-rcf-step3,第 3 步>>。 
|===
. [[verify-rcf-step3]]检查集群端口并切换 RCF。
+
...
.下一步是什么？
验证 RCF 配置后link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["将非根聚合和 NAS 数据 LIF 移回 node2"]。
