...
| 映像版本检查 | 检查要替换的节点是否运行相同版本的 ONTAP。如果 ONTAP 映像版本不同，则任务失败。新节点上必须安装与原节点上相同版本的 ONTAP 9.x。如果新节点安装了不同版本的 ONTAP，则需要在安装后对新控制器进行网络引导。有关如何升级 ONTAP 的说明，请参阅 link:other-references.html["参考资料"] 链接到 _Upgrade ONTAP_。 
...
| 磁盘状态检查 | 如果任何被替换的节点缺少磁盘或出现故障，则任务将失败。如果缺少任何磁盘，请参阅 link:other-references.html["参考资料"] _使用 CLI 进行磁盘和聚合管理_、_使用 CLI 进行逻辑存储管理_ 和 _HA 对管理_，为 HA 对配置存储。 
...
有关许可的详细信息，请参阅 link:other-references.html["参考资料"] 以链接到 _System Administration Reference_。
