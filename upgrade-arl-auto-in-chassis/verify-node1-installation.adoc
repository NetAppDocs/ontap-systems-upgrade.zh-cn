...
== 步骤 1：验证 node1 是否处于仲裁状态

验证 node1 是否处于仲裁状态。然后，恢复控制器更换操作。
...
. 当控制器更换处于暂停状态时，请继续 <<restore_network_configuration_on_node1,还原 node1 上的网络配置>>。




== 步骤 2：恢复 node1 上的网络配置

确认 node1 处于仲裁状态并可与 node2 通信后，请确认 node1 上显示了 node1 的 VLAN ，接口组和广播域。此外，验证所有 node1 网络端口是否均已配置在其正确的广播域中。

.关于此任务
有关创建和重新创建 VLAN 、接口组和广播域的详细信息，请参见 link:other-references.html["参考资料"] 以链接到 _Network Management_ 内容。
...
.下一步是什么？
在验证 node1 安装后，link:restore-key-manager-config-upgraded-node1.html["在已升级的 node1 上恢复密钥管理器配置"]。
