...
---
= 更换 node1 AFF A700 或 FAS9000 控制器和 NVRAM 模块
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
== 步骤 4：在 node1 中安装 ASA A900、AFF A900 或 FAS9500 控制器模块
...
. 中断 AUTOBOOT 后， node1 会在 LOADER 提示符处停止。如果不按时中断 AUTOBOOT 并启动 node1 ，请等待提示，然后输入 Ctrl-C 进入启动菜单。节点在启动菜单处停止后，使用选项 `8`重新启动节点，并在重新启动期间中断 AUTOBOOT。
...
.下一步是什么？
安装 ASA A900、AFF A900 或 FAS9500 控制器和 NVRAM 模块后，link:netboot-node1.html["网络引导 node1"]。
