...
* 此过程很复杂，假设您具有高级 ONTAP 管理技能。您应该阅读并了解 link:guidelines_for_upgrading_controllers_with_arl.html["控制器升级指南"] 和 link:overview_of_the_arl_upgrade.html["ARL 升级序列"]，然后再开始升级。
...
* 如果您要从包含在 link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["支持的系统列表"] 中的停机时间的现有系统升级，则可以通过移动存储或联系技术支持来升级控制器硬件。请参阅 link:other_references.html["参考资料"] 链接到_通过移动卷或存储进行升级_。
