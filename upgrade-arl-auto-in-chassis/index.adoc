...
[role="lead"]
有几种用于升级控制器硬件的聚合重定位 (ARL) 方法。此过程介绍如何通过将现有系统转换为替换系统、保留现有系统机箱、磁盘和磁盘上存储的数据来升级 HA 对中的存储控制器。
...
NOTE: 术语 *node1* 和 *node2* 仅用作本文档中节点名称的引用。遵循此过程时，必须使用节点的实际名称。
...
* 此过程很复杂，假设您具有高级 ONTAP 管理技能。您应该阅读并了解 link:guidelines-for-upgrading-controllers-with-arl.html["控制器升级指南"] 和 link:overview-of-the-arl-upgrade.html["ARL 升级序列"]，然后再开始升级。
...
* 如果您的交换机不受 ONTAP 版本和要升级到的替换系统的支持，请参阅 link:other-references.html["参考资料"] 以链接到 _Hardware Universe_。
...
* 如果您要从包含在 link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["支持的系统列表"] 中的停机时间的现有系统升级，则可以通过移动存储或联系技术支持来升级控制器硬件。请参阅 link:other-references.html["参考资料"] 链接到_通过移动卷或存储进行升级_。
...
.下一步是什么？
了解此 ARL 升级过程后，link:decide-to-use-the-aggregate-relocation-guide.html["决定是否使用此程序"]。
