...
[role="lead"]
具有替换模块的 Node2 现已准备好引导。支持的替换模块列在 link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["支持的系统列表"] 中。
...
.下一步是什么？
使用更换的系统模块启动 node2 后，link:verify-node2-installation.html["验证 node2 安装"]。
