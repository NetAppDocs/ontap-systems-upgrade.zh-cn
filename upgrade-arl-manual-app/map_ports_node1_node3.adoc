...
[role="lead"]
您必须确保 node1 上的物理端口正确映射到 node3 上的物理端口。这允许 node3 在升级后与集群中的其他节点以及与网络进行通信。
...
.. 在执行可达性修复时，ONTAP 尝试将端口放置在正确的广播域中。但是，如果 ONTAP 无法确定端口的可达性，并且该端口与任何现有广播域都不对应，则 ONTAP 尝试为这些端口创建新的广播域。根据需要，如果所有成员端口都成为接口组的成员端口，则可以删除新创建的广播域。删除广播域：
+
...
. [[man_map_1_step5]]从 ONTAP 9.8 开始，如果端口在网络端口可达性修复过程中在广播域之间移动，ONTAP 会自动修改 LIF 的主端口。如果 LIF 的主端口已移至其他节点或未分配，则该 LIF 将显示为移位 LIF。恢复已移位 LIF 的主端口，这些 LIF 的主端口已不存在或已重新定位到其他节点。
+
...
.下一步是什么？
在将端口从 node1 映射到 node3 之后，link:move_nas_lifs_node1_from_node2_node3_verify_san_lifs_node3.html["将 node1 拥有的 NAS 数据 LIF 从 node2 移动到 node3，并在 node3 上验证 SAN LIF"]。
