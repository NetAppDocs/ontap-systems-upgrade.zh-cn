...
. [[man_install3_step16]]如果您安装了 NetApp 存储加密 (NSE) 驱动器，请执行以下步骤：
+
...
+
输入您在此操作步骤前面记录的密码短语和备份信息。请参阅 link:manage_authentication_okm.html["使用板载密钥管理器管理身份验证密钥"]。
...
.下一步是什么？
安装并引导 node3 后，link:set_fc_uta_uta2_config_node3.html["在节点 3 上设置 FC 或 UTA/UTA2 配置"]。
