...
+
如果 node1 和 node2 都不拥有其为当前所有者（但不是主所有者）的聚合，则系统将返回一条类似于以下示例的消息：
...
+
有关该 `network interface revert` 命令的详细信息，请参阅link:other_references.html["参考资料"]以链接到 _ONTAP 9 命令参考_。
...
+
VLAN 通过物理端口配置。如果物理端口发生变化，则需要在以后的过程中重新创建 VLAN。
...
+
如果未为 SVM 配置 iSCSI 或 FC，此命令将显示一条类似于以下示例的消息：
...
+
如果为 SVM 配置了 iSCSI 或 FC，此命令将显示一条类似于以下示例的消息：
...
+
您可能希望将远程管理设备（在本例中为 SP）的网络参数从原始系统中重新用于新节点上的 SP。有关 SP 的详细信息，请参阅 link:other_references.html["参考资料"] 链接到 _System Administration Reference_ 和 _ONTAP 9 Command reference_。
...
+
有关 AutoSupport 的详细信息，请参阅 link:other_references.html["参考资料"] 以链接到 _System Administration Reference_ 和 _ONTAP 9 Command reference_。
...
.下一步是什么？
准备好要升级的节点后，link:manage_authentication_okm.html["使用板载密钥管理器管理身份验证密钥"]。
