...
[role="lead"]
您必须确保 node2 上的物理端口正确映射到 node4 上的物理端口。这使 node4 能够在升级后与集群中的其他节点以及与网络进行通信。
...
| 9.12.1或更高版本 | 跳过此步骤并转到 <<man_map_2_Step3,第 3 步>>。 
|===
+
...
.. 在执行可达性修复时，ONTAP 尝试将端口放置在正确的广播域中。但是，如果 ONTAP 无法确定端口的可达性，并且该端口与任何现有广播域都不对应，则 ONTAP 尝试为这些端口创建新的广播域。根据需要，如果所有成员端口都将成为接口组的成员端口，则可以删除新创建的广播域。删除广播域：
+
...
. [[man_map_2_Step5]]从 ONTAP 9.8 开始，如果端口在网络端口可达性修复过程中在广播域之间移动，ONTAP 会自动修改 LIF 的主端口。如果 LIF 的主端口已移至其他节点或未分配，则该 LIF 将显示为移位 LIF。恢复已移位 LIF 的主端口，这些 LIF 的主端口已不存在或已重新定位到其他节点。
+
...
.下一步是什么？
将端口从 node2 映射到 node4 后，link:move_nas_lifs_node2_from_node3_node4_verify_san_lifs_node4.html["将 node2 拥有的 NAS 数据 LIF 从 node3 移动到 node4，并在 node4 上验证 SAN LIF"]。
