...
WARNING: 准备二手控制器所需的步骤 `wipeconfig`本程序不包含命令，因为本程序假定更换的控制器硬件是全新的，并且未使用过。如果之前使用过替换控制器硬件，则必须联系技术支持，尤其是当控制器在 7 模式下运行Data ONTAP时。
...
.下一步是什么？
在查看使用 ARL 升级控制器的指南后，link:required_tools_documentation.html["查看所需的工具和文档"]。
