...
+
转到 link:other_references.html["参考资料"] 以链接到 _System Administration Reference_ 获取有关 SP 的信息，以及 _ONTAP 9 Command reference_ 获取有关  `system service- processor network modify` 命令的详细信息。
...
.下一步是什么？
确认新控制器已正确设置后，link:set_up_storage_encryption_new_controller.html["在新的控制器模块上设置存储加密"]。
