...
有关存储聚合重定位命令的详细信息，请参见 link:other_references.html["参考资料"] 链接到 _Disk and aggregate management with the CLI_ 内容和 _ONTAP 9 Command reference_。
...
.下一步是什么？
将非根聚合从 node1 重新定位到 node2 后，link:move_nas_lifs_node1_node2.html["将 node1 拥有的 NAS 数据 LIF 移至 node2"]。
