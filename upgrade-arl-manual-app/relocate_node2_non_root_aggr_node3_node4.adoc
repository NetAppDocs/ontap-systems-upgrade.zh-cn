...
有关存储聚合重定位命令的详细信息，请参见 link:other_references.html["参考资料"] 链接到 _使用 CLI 进行磁盘和聚合管理_ 和 _ONTAP 9 命令参考_。
...
.下一步是什么？
将 node2 的非根聚合从 node3 重新定位到 node4 后，link:manage_authentication_kmip.html["使用 KMIP 服务器管理身份验证"]。
