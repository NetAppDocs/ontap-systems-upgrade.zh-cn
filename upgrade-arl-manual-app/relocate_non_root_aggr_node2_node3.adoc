...
有关存储聚合重定位命令的详细信息，请转到 link:other_references.html["参考资料"] 链接到 _使用 CLI 进行磁盘和聚合管理_ 和 _ONTAP 9 命令参考_。
...
.下一步是什么？
将非根聚合从 node2 重新定位到 node3 后，link:move_nas_lifs_node2_node3.html["将 node2 拥有的 NAS 数据 LIF 移至 node3"]。
