...
* 将原始系统转换为驱动器架并移动存储：
+
** link:upgrade/upgrade_aff_a250_to_aff_a400_ndu_upgrade_workflow.html["将 AFF A250 转换为驱动器架，并将存储移动到 AFF A400"]
** link:upgrade/convert-to-drive-shelf.html["将 AFF A150、AFF A220 或 FAS2820 转换为驱动器架，并将存储转移到支持的替换系统"]
...
| 我是否拥有一个集成系统，并且想要仅通过更换控制器模块进行升级，而保留现有的机箱和磁盘？ | link:upgrade-arl-auto-in-chassis/index.html["通过使用 ARL 进行升级，保留相同的机箱、磁盘和存储在磁盘上的数据"]。 
